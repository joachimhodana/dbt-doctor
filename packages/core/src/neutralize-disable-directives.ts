    ...
    for (const [absolutePath, originalContent] of originalContents) {
      try {
        ...
        process.stderr.write(
          `[dbt-doctor] Failed to restore inline disable directives in ${absolutePath}: ${error instanceof Error ? error.message : String(error)}\n` +
            `[dbt-doctor] Run: git checkout -- ${absolutePath}\n`,
        );
      }
    }
  };
