...
- [`29b7229`](https://github.com/joachimhodana/dbt-doctor/commit/29b7229ea144cfe80c4401391eed3aa035071bcd) Thanks [@aidenybai](https://github.com/aidenybai)! - Add `dbt-doctor-rules` to `dependencies` so it is installed
  ...
  dependency list, causing `ERR_MODULE_NOT_FOUND` on `npx dbt-doctor`.
...
- [`10d5de8`](https://github.com/joachimhodana/dbt-doctor/commit/10d5de804fe9c03fa9f18e5350bb26965a5108ac) Thanks [@aidenybai](https://github.com/aidenybai)! - Fix workspace packages not being bundled into dist, causing
  `ERR_MODULE_NOT_FOUND: Cannot find package '@dbt-doctor/core'`
  when running the published CLI.

- [#284](https://github.com/joachimhodana/dbt-doctor/pull/284) [`b34c5c4`](https://github.com/joachimhodana/dbt-doctor/commit/b34c5c4db28539d9407fc08600e0e8c28dea67cd) Thanks [@aidenybai](https://github.com/aidenybai)! - Harden glob pattern compilation against ReDoS:
  ...
  - Reject obviously pathological patterns early with a clear `InvalidGlobPatternError` carrying the offending pattern and a human-readable reason, instead of crashing the scan. Limits live in `@dbt-doctor/core/constants` (`MAX_GLOB_PATTERN_LENGTH_CHARS = 1024`, `MAX_GLOB_PATTERN_WILDCARD_COUNT = 24`) and bound worst-case work regardless of the underlying engine. Real-world ignore patterns like `**/foo/**/bar/**/*.tsx` sit well under the cap.
  - Surface invalid `ignore.files` and `ignore.overrides[*].files` entries as `[dbt-doctor] …` warnings on stderr and skip just the bad pattern, so a single typo no longer takes the whole scan down.
...
- [#202](https://github.com/joachimhodana/dbt-doctor/pull/202) [`53fa4df`](https://github.com/joachimhodana/dbt-doctor/commit/53fa4dffe837e0157fb850fef700fccaaec191ea) Thanks [@aidenybai](https://github.com/aidenybai)! - Detect the project's Tailwind version (`tailwindcss` in `package.json`,
...
- Updated dependencies []:
  - dbt-doctor-rules@0.2.0-beta.6

## 0.2.0-beta.5

### Patch Changes

- [#271](https://github.com/joachimhodana/dbt-doctor/pull/271) [`7a7ec84`](https://github.com/joachimhodana/dbt-doctor/commit/7a7ec84fad631d96f70279394be5f086b8424d17) - **Per-surface diagnostic controls (CLI).** New
  ...
  `@dbt-doctor/core` surface filter into `cli/commands/inspect.ts`
...
- Inherits the rule-fix wave from
  `dbt-doctor-rules@0.2.0-beta.5` (rules are bundled into
  the CLI):
  `no-secrets-in-client-code` scoping
  ([#252](https://github.com/joachimhodana/dbt-doctor/pull/252)),
  `nextjs-no-side-effect-in-get-handler` safe local bindings
  ([#260](https://github.com/joachimhodana/dbt-doctor/pull/260)),
  `async-defer-await` false-positive fixes
  ([#265](https://github.com/joachimhodana/dbt-doctor/pull/265)),
  `js-length-check-first` `&&`-chain detection
  ([#269](https://github.com/joachimhodana/dbt-doctor/pull/269)),
  `async-parallel` test / browser-fixture suppression
  ([#270](https://github.com/joachimhodana/dbt-doctor/pull/270)),
  `js-combine-iterations` lazy `Iterator` skip
  ([#272](https://github.com/joachimhodana/dbt-doctor/pull/272)), and
  `no-prevent-default` framework awareness
  ([#274](https://github.com/joachimhodana/dbt-doctor/pull/274)).
...
- Updated dependencies [[`529015d`](https://github.com/joachimhodana/dbt-doctor/commit/529015d1d89441c4708f49413ecd540db7c04255)]:
  - dbt-doctor-rules@0.2.0-beta.5

## 0.2.0-beta.4

### Patch Changes

- Add `dbt-doctor-rules` to `dependencies` so it is installed
  alongside the CLI. The bundler correctly externalises the plugin (oxlint
  loads it by file path at runtime) but it was missing from the published
  dependency list, causing `ERR_MODULE_NOT_FOUND` on `npx dbt-doctor`.
- Updated dependencies []:
  - dbt-doctor-rules@0.2.0-beta.4

## 0.2.0-beta.3

### Patch Changes

- [`10d5de8`](https://github.com/joachimhodana/dbt-doctor/commit/10d5de804fe9c03fa9f18e5350bb26965a5108ac) - Fix workspace packages
  (`@dbt-doctor/core`, `@dbt-doctor/project-info`,
  `@dbt-doctor/types`) not being bundled into the published `dist/`
  output, which caused
  `ERR_MODULE_NOT_FOUND: Cannot find package '@dbt-doctor/core'`
  on `npx dbt-doctor` after the package extraction in beta.2.
  Vite config now treats the workspace dependencies as bundle-time
  inputs.

- Inherits the
  [#253](https://github.com/joachimhodana/dbt-doctor/pull/253) `no-barrel-import`
  index-resolution fix from
  `dbt-doctor-rules@0.2.0-beta.3` (rules are bundled into
  the CLI).

- Updated dependencies []:
  - dbt-doctor-rules@0.2.0-beta.3

## 0.2.0-beta.2

### Minor Changes

- [#249](https://github.com/joachimhodana/dbt-doctor/pull/249) [`f0198e2`](https://github.com/joachimhodana/dbt-doctor/commit/f0198e2f2d9560a15bdb4a78f4a378ca2ac5fcdd) - **Internal-package extraction.** The CLI no longer vendors project
  detection, the oxlint runner, scoring, or the shared type layer
  inline - those modules now live in
  `@dbt-doctor/types`, `@dbt-doctor/project-info`, and
  `@dbt-doctor/core` and are consumed as workspace dependencies.
  Bundled into `dist/` on publish (see also
  [#253](https://github.com/joachimhodana/dbt-doctor/pull/253) bundler
  follow-up in beta.3). The `dbt-doctor`, `dbt-doctor inspect`,
  and `dbt-doctor install` binaries are surface-compatible with
  0.1.6.

- [#250](https://github.com/joachimhodana/dbt-doctor/pull/250) [`6e2ee9d`](https://github.com/joachimhodana/dbt-doctor/commit/6e2ee9d474fddbde4c1246ff65b2f3e5bb3a42fc) - **CLI reorganised.** `src/cli/` is now split into `commands/` +
...
- [#208](https://github.com/joachimhodana/dbt-doctor/pull/208) [`8556b31`](https://github.com/joachimhodana/dbt-doctor/commit/8556b31d8e4e165f791db0aa60a6b038b18ec777) - **User-feedback sweep.** Reduce false positives across the design /
  Tailwind / state-and-effects rule groups, surface per-rule scoring
  contributions in `dbt-doctor inspect`, and add `--severity` /
  `--rule-set` CLI options plus their `dbt-doctor.config.json`
  counterparts. Closes the bulk of the feedback collected on 0.1.x.

- [#174](https://github.com/joachimhodana/dbt-doctor/pull/174) - Forward
  `reactMajorVersion` through the programmatic `diagnose()` entry
  point so embedders running dbt-doctor inside their own pipeline
  (Vercel AI Code Review sandbox and friends) get the same React-19
  rule gating the CLI gets.

- [#202](https://github.com/joachimhodana/dbt-doctor/pull/202) [`53fa4df`](https://github.com/joachimhodana/dbt-doctor/commit/53fa4dffe837e0157fb850fef700fccaaec191ea) - Detect the project's Tailwind version (`tailwindcss` in `package.json`,
  including pnpm and Bun catalog references) and gate Tailwind-aware
  rules on it. `design-no-redundant-size-axes` (which suggests collapsing
  `w-N h-N` → `size-N`) now stays silent on Tailwind v3.0 … v3.3 - those
  versions predate the `size-N` shorthand and the suggestion would
...
- [`e9e4217`](https://github.com/joachimhodana/dbt-doctor/commit/e9e4217711341cf2bfb7c1b11f37caaae44df7c3) - Harden `discover-project` and `resolve-diagnose-target`: tighter
...
- [`b06b768`](https://github.com/joachimhodana/dbt-doctor/commit/b06b768) ([#193](https://github.com/joachimhodana/dbt-doctor/pull/193)) - `diagnose()` now falls back to the first nested React subproject when the
...
- [#200](https://github.com/joachimhodana/dbt-doctor/pull/200) - Typed
...
- [#201](https://github.com/joachimhodana/dbt-doctor/pull/201) - Integrate
  ...
  alongside dbt-doctor's own state-and-effects rules.
...
- [#196](https://github.com/joachimhodana/dbt-doctor/pull/196) - Match
  `dbt-doctor-disable*` suppression comments that carry descriptive
  trailing text (e.g. `// dbt-doctor-disable-next-line rule -- why`)
  instead of requiring a bare comment. Resolves
  [#159](https://github.com/joachimhodana/dbt-doctor/issues/159).

- [#198](https://github.com/joachimhodana/dbt-doctor/pull/198) - Expose
  `--why` as a documented public alias for `--explain` in the CLI.
  Resolves [#161](https://github.com/joachimhodana/dbt-doctor/issues/161).

- [#195](https://github.com/joachimhodana/dbt-doctor/pull/195) - The
  ...
  Resolves [#190](https://github.com/joachimhodana/dbt-doctor/issues/190).

- [#197](https://github.com/joachimhodana/dbt-doctor/pull/197) - Docs:
  clarify that `ignore.overrides` covers per-file rule ignores.

- [#199](https://github.com/joachimhodana/dbt-doctor/pull/199) - Docs:
...
- [`a63d5d5`](https://github.com/joachimhodana/dbt-doctor/commit/a63d5d5d0bbff26e7367a0b6d634aeb089a935ef) - CLI scan output reformat. Adds `utils/wrap-indented-text.ts` for
...
- [#184](https://github.com/joachimhodana/dbt-doctor/pull/184) - Add a
  ...
  [#183](https://github.com/joachimhodana/dbt-doctor/issues/183).

- [#182](https://github.com/joachimhodana/dbt-doctor/pull/182) - Restore
...
- [#181](https://github.com/joachimhodana/dbt-doctor/pull/181) - Website
  fix: keep the diagnostic count next to the rule name on narrow widths
  in the leaderboard / diagnostic listings.

- [`cca5808`](https://github.com/joachimhodana/dbt-doctor/commit/cca5808) - Promote `react-hooks-js/*` diagnostics to errors so projects with
  React Hooks rule violations no longer pass with a clean score.

- [`9ee3a6d`](https://github.com/joachimhodana/dbt-doctor/commit/9ee3a6d) - Refresh the website's terminal demo to match the new CLI output
...
- [`6ddb02c`](https://github.com/joachimhodana/dbt-doctor/commit/6ddb02c6b08fbfb06e2429d3cabd338c91891cd6) - Polish follow-up to the 0.1.1 CLI redesign. Consolidates duplicated
...
- [#178](https://github.com/joachimhodana/dbt-doctor/pull/178) - CLI
...
- d71a6bf: feat(dbt-doctor): ship rules as an ESLint plugin (`dbt-doctor/eslint-plugin`)

  The same React Doctor rule set that powers the CLI scan and the
  `dbt-doctor/oxlint-plugin` export is now available as a first-class
  ...
  hook already speaks ESLint - no separate `dbt-doctor` invocation
  needed.

  ```js
  // eslint.config.js
  import dbtDoctor from "dbt-doctor/eslint-plugin";

  export default [
    dbtDoctor.configs.recommended,
    dbtDoctor.configs.next, // composable framework presets
    dbtDoctor.configs["react-native"],
    dbtDoctor.configs["tanstack-start"],
    dbtDoctor.configs["tanstack-query"],
    // dbtDoctor.configs.all, // every rule at dbt-doctor's default severity
  ];
  ```
  ...
  dbt-doctor CLI emits to oxlint, so behavior stays in lock-step
  between engines. You can also cherry-pick individual rules under the
  `dbt-doctor/*` namespace.
  ...
  [#143](https://github.com/joachimhodana/dbt-doctor/issues/143).

- d71a6bf: feat(dbt-doctor): adopt the project's existing oxlint / eslint config and factor those rules into the score
  ...
  dbt-doctor now folds that config into the same scan via oxlint's
  `extends` field. The user's existing rules fire alongside the curated
  dbt-doctor rule set, and the resulting diagnostics count toward the
  ...
  **Behavior change on upgrade.** Projects with an existing
  ...
  `"adoptExistingLintConfig": false` in `dbt-doctor.config.json` (or the
  `"dbtDoctor"` key in `package.json`) to preserve the previous
  behavior. `customRulesOnly: true` also implies opt-out, since that mode
  runs only the `dbt-doctor/*` plugin.

  **Resilience.** If oxlint can't load the user's config (broken JSON,
  missing plugin, unknown rule name), dbt-doctor logs the reason on
  ...
  **Coverage broadened.** Diagnostics on `.ts` and `.js` files are now
  ...
  / `.jsx`). This affects dbt-doctor's own JS-performance / bundle-size
  ...
  **Limitations.** Only JSON configs are picked up: oxlint's `extends`
  ...
  category-level enables (`"categories": {...}`) do not - dbt-doctor's
  local categories block always wins. Closes #143.

- d71a6bf: feat(dbt-doctor): add 11 new lint rules - 3 state / correctness, 8 design system

  **3 new state / correctness rules** (all `warn`):

  - `dbt-doctor/no-direct-state-mutation` - flags `state.foo = x` and
    ...
    false-positive.
  - `dbt-doctor/no-set-state-in-render` - flags only **unconditional**
    top-level setter calls so the canonical
    `if (prev !== prop) setPrev(prop)` derive-from-props pattern stays
    clean.
  - `dbt-doctor/no-uncontrolled-input` - catches `<input value={…}>`
  ...
  **8 new design-system rules in `react-ui.ts`** (all `warn`):
  ...
  - `dbt-doctor/design-no-bold-heading` -
    `font-bold` / `font-extrabold` / `font-black` or inline
    `fontWeight ≥ 700` on `h1`–`h6`.
  - `dbt-doctor/design-no-redundant-padding-axes` - collapse
    `px-N py-N` → `p-N`.
  - `dbt-doctor/design-no-redundant-size-axes` - collapse `w-N h-N` →
    `size-N`.
  - `dbt-doctor/design-no-space-on-flex-children` - use `gap-*` over
    `space-*-*`.
  - `dbt-doctor/design-no-em-dash-in-jsx-text` - em dashes in JSX
    text.
  - `dbt-doctor/design-no-three-period-ellipsis` - `Loading...` →
    `Loading…`.
  - `dbt-doctor/design-no-default-tailwind-palette` -
    `indigo-*` / `gray-*` / `slate-*` reads as the Tailwind template
    default; reports every offending token in the className (not just
    the first).
  - `dbt-doctor/design-no-vague-button-label` - `OK` / `Continue` /
...
- d71a6bf: remove(dbt-doctor): drop browser entrypoints, browser CLI, and the
  `dbt-doctor-browser` workspace package

  **Removed package exports.** `dbt-doctor/browser` and
  `dbt-doctor/worker` are no longer published. Imports of either subpath
  ...
  `dbt-doctor@0.0.47` or vendor the relevant modules from the
  `archive/browser` git branch.

  **Removed CLI subcommand.** `dbt-doctor browser …` (`start`, `stop`,
  ...
  **Removed companion package.** The `dbt-doctor-browser` npm package
  ...
  **Why.** The browser surface area was unused inside the monorepo (the
  ...
  consumers. Removing it tightens what `dbt-doctor` is responsible for:
  the diagnostics CLI, the Node `dbt-doctor/api`, and the
  `dbt-doctor/eslint-plugin` / `dbt-doctor/oxlint-plugin` exports.
...
- 2aebfa6: fix(dbt-doctor): support block comment forms of `dbt-doctor-disable-line` / `dbt-doctor-disable-next-line`

  The inline-suppression matcher previously only recognized line comments
  (`// dbt-doctor-disable-…`). Block comments - including the JSX form
  `{/* dbt-doctor-disable-next-line … */}`, which is the only suppression
  form legal directly inside JSX - were silently ignored, forcing users to
  write `{/* // dbt-doctor-disable-line … */}` as a workaround. Both forms
...
- 2aebfa6: fix(dbt-doctor): stop flagging `useState` as `useRef` when state reaches render through `useMemo`, derived values, or context `value`
...
- [#148](https://github.com/joachimhodana/dbt-doctor/pull/148) ([`3f5c031`](https://github.com/joachimhodana/dbt-doctor/commit/3f5c031)) - Add the (now-removed in 0.1.0) `dbt-doctor browser` CLI subcommand
...
- [#152](https://github.com/joachimhodana/dbt-doctor/pull/152) ([`8f10098`](https://github.com/joachimhodana/dbt-doctor/commit/8f10098)) - 4 new React 18→19 migration rules: pre-flight checks for the
  `forwardRef`-deprecation / new context API / new ref-callback
  cleanup / `use()` adoption migration paths.

- [#154](https://github.com/joachimhodana/dbt-doctor/pull/154) ([`276ea2f`](https://github.com/joachimhodana/dbt-doctor/commit/276ea2f)) - Add `prefer-use-sync-external-store` rule. Catches `useEffect`-based
  subscriptions that should be `useSyncExternalStore` (concurrent-mode
  safe, tearing-resistant).

- [#155](https://github.com/joachimhodana/dbt-doctor/pull/155) ([`2240b1f`](https://github.com/joachimhodana/dbt-doctor/commit/2240b1f)) - Add `no-event-trigger-state` rule. Flags state that is only ever
  written from an event handler and only read from the rendered JSX
  return - a frequent prop-derivation antipattern.

- [#156](https://github.com/joachimhodana/dbt-doctor/pull/156) ([`4b92f50`](https://github.com/joachimhodana/dbt-doctor/commit/4b92f50)) - Add `no-effect-chain` rule. Flags `useEffect` chains where one
...
- [#157](https://github.com/joachimhodana/dbt-doctor/pull/157) ([`0be99ad`](https://github.com/joachimhodana/dbt-doctor/commit/0be99ad)) - Comprehensive useEffect analyzer: three new rules
  (`no-mutable-in-deps`, `no-mirror-prop-effect`, `effect-needs-cleanup`)
  plus shared dependency-tracking infrastructure.

- [#162](https://github.com/joachimhodana/dbt-doctor/pull/162) ([`945138d`](https://github.com/joachimhodana/dbt-doctor/commit/945138d)) - Gate `prefer-use-effect-event` behind React 19+ so the suggestion
  doesn't fire on React 18 projects (where `useEffectEvent` is not
  available).

- [#163](https://github.com/joachimhodana/dbt-doctor/pull/163) ([`c20857e`](https://github.com/joachimhodana/dbt-doctor/commit/c20857e)) - `no-effect-event-handler` honors empty-frame barriers in prop-stack
  lookups so callbacks hoisted out of effects don't inherit the
  surrounding effect classification.

- [#165](https://github.com/joachimhodana/dbt-doctor/pull/165) ([`78db3b2`](https://github.com/joachimhodana/dbt-doctor/commit/78db3b2)) - Multi-line JSX and stacked suppression comments, per-file rule
  overrides, and near-miss hints for misspelled rule ids.

- [#166](https://github.com/joachimhodana/dbt-doctor/pull/166) ([`8745c34`](https://github.com/joachimhodana/dbt-doctor/commit/8745c34)) - Suppression follow-ups: audit-mode handling, overrides-config
...
- [#167](https://github.com/joachimhodana/dbt-doctor/pull/167) ([`d3e26d6`](https://github.com/joachimhodana/dbt-doctor/commit/d3e26d6)) - Refactor: consolidate state-and-effects rule plumbing.

- [#169](https://github.com/joachimhodana/dbt-doctor/pull/169) ([`50d08fd`](https://github.com/joachimhodana/dbt-doctor/commit/50d08fd)) - AGENTS.md compliance pass on the state-and-effects rule directory.

- [#170](https://github.com/joachimhodana/dbt-doctor/pull/170) ([`97cb1bb`](https://github.com/joachimhodana/dbt-doctor/commit/97cb1bb)) - Collapse non-verbose CLI diagnostics to the top 3 rules so the
  default scan output stays scannable on large projects; `--verbose`
  restores the full listing.

- [#172](https://github.com/joachimhodana/dbt-doctor/pull/172) ([`2a1b0ae`](https://github.com/joachimhodana/dbt-doctor/commit/2a1b0ae)) - Tighten state-and-effects rules against false positives across the
  rules-of-hooks / handler-detection / render-reachable code paths.

- [#174](https://github.com/joachimhodana/dbt-doctor/pull/174) ([`4fb4d27`](https://github.com/joachimhodana/dbt-doctor/commit/4fb4d27)) - Forward `reactMajorVersion` through the programmatic `diagnose()`
  entry point so embedders get the same React-19 rule gating the CLI
  uses.

- [#177](https://github.com/joachimhodana/dbt-doctor/pull/177) ([`01c38a7`](https://github.com/joachimhodana/dbt-doctor/commit/01c38a7)) - Harden rules against prototype-pollution false positives and quiet
...
- 6a0e6d6: chore(dbt-doctor): bump oxlint to ^1.62.0
...
- dbf200d: fix(dbt-doctor): filter React Compiler rules to those the loaded `eslint-plugin-react-hooks` actually exports
...
- c13a8df: fix(dbt-doctor): skip React Compiler rules when `eslint-plugin-react-hooks` isn't installed
...
- 6b07924: `dbt-doctor install` now delegates skill installation to
...
- [`57467cd`](https://github.com/joachimhodana/dbt-doctor/commit/57467cd) - Patch follow-up to the 0.0.43 ignore-respecting refactor: misc
...
- [`1fdc9a0`](https://github.com/joachimhodana/dbt-doctor/commit/1fdc9a0) - Patch follow-up to the 0.0.39 browser-entrypoint work: misc
  bundling fixes for the now-removed `dbt-doctor/browser` and
  `dbt-doctor/worker` subpath exports.

## 0.0.40

### Patch Changes

- [`874f7bc`](https://github.com/joachimhodana/dbt-doctor/commit/874f7bc) - Publishing-pipeline retry of 0.0.39 (no code delta).

## 0.0.39

### Patch Changes

- [#134](https://github.com/joachimhodana/dbt-doctor/pull/134) ([`7da4ce4`](https://github.com/joachimhodana/dbt-doctor/commit/7da4ce4)) - Fix `TypeError: issues.files is not iterable` crash during dead-code
...
- [`5238e7b`](https://github.com/joachimhodana/dbt-doctor/commit/5238e7b) / [`061a794`](https://github.com/joachimhodana/dbt-doctor/commit/061a794) - Add the `dbt-doctor/browser` entrypoint and a `diagnoseBrowser` /
...
- [`b5519b6`](https://github.com/joachimhodana/dbt-doctor/commit/b5519b6) - Inject the browser scorer at build time so the bundled
  `dbt-doctor/browser` output omits the proxy-fetch / Node-only score
  path.
...
- [`8b0485a`](https://github.com/joachimhodana/dbt-doctor/commit/8b0485a) - GitHub Action improvements (input validation + step output cleanups),
  clickable file paths in CLI diagnostic output (terminal hyperlinks
  via OSC-8), and a website hydration fix.

- [`bb5188f`](https://github.com/joachimhodana/dbt-doctor/commit/bb5188f) - Document every config option supported by `dbt-doctor.config.json`
  in the README.

- [`100731c`](https://github.com/joachimhodana/dbt-doctor/commit/100731c) - `install-skill` and `detect-agents` formatting fixes so CI's
...
- [`f1bd776`](https://github.com/joachimhodana/dbt-doctor/commit/f1bd776) - Republish 0.0.36 after a botched skill payload - the
...
- [#131](https://github.com/joachimhodana/dbt-doctor/pull/131) ([`ff5b637`](https://github.com/joachimhodana/dbt-doctor/commit/ff5b637)) - Frontend design-quality rules (initial cut of the design-system
  rule family - bold headings, redundant padding/size axes, vague
  button labels, etc.). Expanded substantially in 0.1.0.

- [`074f854`](https://github.com/joachimhodana/dbt-doctor/commit/074f854) - Add the `dbt-doctor install` subcommand that installs the
...
- [`7136aa5`](https://github.com/joachimhodana/dbt-doctor/commit/7136aa5) - Republish 0.0.34 after a packaging hiccup; no code delta.

## 0.0.34

### Patch Changes

- [#124](https://github.com/joachimhodana/dbt-doctor/pull/124) ([`0eb635a`](https://github.com/joachimhodana/dbt-doctor/commit/0eb635a)) - Add TanStack Start rule family (router conventions, server-function
  hygiene, `useServerFn` adoption hints).

- [#129](https://github.com/joachimhodana/dbt-doctor/pull/129) ([`47105e9`](https://github.com/joachimhodana/dbt-doctor/commit/47105e9)) - Fix false positives for Next.js redirect guidance and React Compiler
  detection.
...
- [`87d4b86`](https://github.com/joachimhodana/dbt-doctor/commit/87d4b86) - Republish 0.0.32 after a packaging hiccup; no code delta.

## 0.0.32

### Patch Changes

- [#120](https://github.com/joachimhodana/dbt-doctor/pull/120) ([`10bd788`](https://github.com/joachimhodana/dbt-doctor/commit/10bd788)) - Address multiple GitHub issues (#117, #113, #119, #106): assorted
  rule false-positive fixes, CLI option polish, and detection
  hardening.

- [#122](https://github.com/joachimhodana/dbt-doctor/pull/122) ([`901fce5`](https://github.com/joachimhodana/dbt-doctor/commit/901fce5)) - Restrict setter detection to direct `Identifier` callees so dynamic
...
- [#108](https://github.com/joachimhodana/dbt-doctor/pull/108) ([`1aef45a`](https://github.com/joachimhodana/dbt-doctor/commit/1aef45a)) - Resolve 7 GitHub issues across catalog resolution, file-ignore
  semantics, CLI ergonomics, React Native detection, Next.js
  detection, the `--offline` flag, and monorepo discovery.

- [#110](https://github.com/joachimhodana/dbt-doctor/pull/110) ([`720f421`](https://github.com/joachimhodana/dbt-doctor/commit/720f421)) - Apply `ignore.files` as a pre-filter so ignored files are skipped
...
- [`c405f4a`](https://github.com/joachimhodana/dbt-doctor/commit/c405f4a) - Resolve multiple GitHub issues (#71, #72, #76, #77, #83, #84, #86,
  #87, #89, #92, #93, #94): broad rule false-positive sweep across
  detection, scoring, and rule output formatting.

- [`97b21f1`](https://github.com/joachimhodana/dbt-doctor/commit/97b21f1) - Replace `fs.existsSync` with the shared `isFile` utility for
...
- [#63](https://github.com/joachimhodana/dbt-doctor/pull/63) ([`9f51b9d`](https://github.com/joachimhodana/dbt-doctor/commit/9f51b9d)) - GitHub Action: add a `diff` input so the action can scan only files
  changed in the PR, plus optional PR-comment posting from the action
  step.

- [#64](https://github.com/joachimhodana/dbt-doctor/pull/64) ([`79af7ca`](https://github.com/joachimhodana/dbt-doctor/commit/79af7ca)) - Detect Expo and React Native and enable the RN-specific rule
  family when a project targets them.

- [#68](https://github.com/joachimhodana/dbt-doctor/pull/68) ([`1947859`](https://github.com/joachimhodana/dbt-doctor/commit/1947859)) - Rule / detection false-positive fixes (issue rollup).

## 0.0.28

### Patch Changes

- [`bd949cc`](https://github.com/joachimhodana/dbt-doctor/commit/bd949cc) - Bump the Node version requirement, enhance the linting process with
...
- [`370ea4c`](https://github.com/joachimhodana/dbt-doctor/commit/370ea4c) - Refactor CLI option handling and improve automated-environment
  detection (CI / agent contexts).

- [`051a02c`](https://github.com/joachimhodana/dbt-doctor/commit/051a02c) - Score-calculation refactor: extract shared constants and add the
  proxy-fetch fallback used by the in-browser scorer.

- [`bf21a87`](https://github.com/joachimhodana/dbt-doctor/commit/bf21a87) - Fix `--fix` deeplink rendering issues introduced when the install
...
- [`7716d6c`](https://github.com/joachimhodana/dbt-doctor/commit/7716d6c) - Integrate the SKILL.md installation prompt directly into the CLI
...
- [#44](https://github.com/joachimhodana/dbt-doctor/pull/44) ([`dcf4276`](https://github.com/joachimhodana/dbt-doctor/commit/dcf4276)) - Remove the silent global install introduced in 0.0.9; the CLI no
  longer auto-installs itself globally on first run. Resolves
  [#43](https://github.com/joachimhodana/dbt-doctor/issues/43).

- [`7e20da1`](https://github.com/joachimhodana/dbt-doctor/commit/7e20da1) - Refactor the diagnostic payload structure used by the
  score-estimation API and tighten its validation.

- [`da83168`](https://github.com/joachimhodana/dbt-doctor/commit/da83168) - Enhance the React Doctor skill installation script with detailed
...
- [#33](https://github.com/joachimhodana/dbt-doctor/pull/33) ([`c2b687a`](https://github.com/joachimhodana/dbt-doctor/commit/c2b687a)) - Detect React Compiler in Expo projects.

- [#37](https://github.com/joachimhodana/dbt-doctor/pull/37) ([`7f49833`](https://github.com/joachimhodana/dbt-doctor/commit/7f49833)) - Chore: add node.js to leaderboard.

## 0.0.23

### Patch Changes

- [`c05f4b0`](https://github.com/joachimhodana/dbt-doctor/commit/c05f4b0) / [`b33976c`](https://github.com/joachimhodana/dbt-doctor/commit/b33976c) - Patch sweep - assorted rule and CLI false-positive fixes.

## 0.0.22

### Patch Changes

- [`84bb6d5`](https://github.com/joachimhodana/dbt-doctor/commit/84bb6d5) / [`61406e0`](https://github.com/joachimhodana/dbt-doctor/commit/61406e0) / [`1b07fa2`](https://github.com/joachimhodana/dbt-doctor/commit/1b07fa2) / [`0299fc4`](https://github.com/joachimhodana/dbt-doctor/commit/0299fc4) - Patch sweep - rule false-positive fixes and formatting follow-ups.

## 0.0.21

### Patch Changes

- [`73da9e2`](https://github.com/joachimhodana/dbt-doctor/commit/73da9e2) - Add the `--offline` flag so the CLI skips network calls (telemetry,
...
- [`45a3d33`](https://github.com/joachimhodana/dbt-doctor/commit/45a3d33) / [`d90a60d`](https://github.com/joachimhodana/dbt-doctor/commit/d90a60d) / [`b2d0b64`](https://github.com/joachimhodana/dbt-doctor/commit/b2d0b64) - Surface stderr from `oxlint` / `knip` invocations so failures
...
- [`b8cc03a`](https://github.com/joachimhodana/dbt-doctor/commit/b8cc03a) / [`2d2f7a1`](https://github.com/joachimhodana/dbt-doctor/commit/2d2f7a1) - Update CLI options, enhance configuration documentation, and
  miscellaneous fixes.
...
- [#17](https://github.com/joachimhodana/dbt-doctor/pull/17) ([`64d837b`](https://github.com/joachimhodana/dbt-doctor/commit/64d837b)) - Add `typescript` as a direct dependency to resolve the knip peer
...
- [`5c2119b`](https://github.com/joachimhodana/dbt-doctor/commit/5c2119b) / [`116d6cd`](https://github.com/joachimhodana/dbt-doctor/commit/116d6cd) - Enhance error handling in dead-code detection so a knip failure no
...
- [`06fb14e`](https://github.com/joachimhodana/dbt-doctor/commit/06fb14e) - Improve error handling in the linting and dead-code analysis paths
  so failures log a useful message instead of a stack trace.

- [`595ca55`](https://github.com/joachimhodana/dbt-doctor/commit/595ca55) - Remove the video package from `main` (kept on a separate branch for
  asset generation).
...
- [`e8cfef0`](https://github.com/joachimhodana/dbt-doctor/commit/e8cfef0) / [`1410650`](https://github.com/joachimhodana/dbt-doctor/commit/1410650) - Update `dbt-doctor` package configuration / constants and refresh
...
- [`90ffa0a`](https://github.com/joachimhodana/dbt-doctor/commit/90ffa0a) - Add `llms.txt` so models discovering the package via the npm
  registry can find structured docs.

- [`e218d63`](https://github.com/joachimhodana/dbt-doctor/commit/e218d63) - Format the leaderboard data files to satisfy CI `format:check`.

## 0.0.13

### Patch Changes

- [#11](https://github.com/joachimhodana/dbt-doctor/pull/11) ([`2d2f779`](https://github.com/joachimhodana/dbt-doctor/commit/2d2f779)) - React Doctor branding refresh: theme-aware README logo + asset
  refresh. Reverted in [`28d820a`](https://github.com/joachimhodana/dbt-doctor/commit/28d820a) when the assets failed to render
  correctly on npm.

- [#12](https://github.com/joachimhodana/dbt-doctor/pull/12) ([`5fc130d`](https://github.com/joachimhodana/dbt-doctor/commit/5fc130d)) - Fix the favicon badge ring cutout.

- [#14](https://github.com/joachimhodana/dbt-doctor/pull/14) ([`67bad8f`](https://github.com/joachimhodana/dbt-doctor/commit/67bad8f)) - Set the Twitter image to the brand banner.

## 0.0.12

### Patch Changes

- [`b200689`](https://github.com/joachimhodana/dbt-doctor/commit/b200689) / [`4519747`](https://github.com/joachimhodana/dbt-doctor/commit/4519747) / [`7f0b4d2`](https://github.com/joachimhodana/dbt-doctor/commit/7f0b4d2) / [`6d1ae5e`](https://github.com/joachimhodana/dbt-doctor/commit/6d1ae5e) / [`5688c1f`](https://github.com/joachimhodana/dbt-doctor/commit/5688c1f) / [`6dd481a`](https://github.com/joachimhodana/dbt-doctor/commit/6dd481a) / [`ce8437e`](https://github.com/joachimhodana/dbt-doctor/commit/ce8437e) - Iteration sweep on detection / output formatting / README copy
...
- [`db319d0`](https://github.com/joachimhodana/dbt-doctor/commit/db319d0) / [`358750a`](https://github.com/joachimhodana/dbt-doctor/commit/358750a) - Environment-variable handling fix: thread the documented
...
- [`e5ef934`](https://github.com/joachimhodana/dbt-doctor/commit/e5ef934) - "Almost ready" milestone - the rule pipeline + scoring + CLI surface
...
- [#2](https://github.com/joachimhodana/dbt-doctor/pull/2) ([`1ae6094`](https://github.com/joachimhodana/dbt-doctor/commit/1ae6094)) - Improve the `--prompt` clipboard output emitted for agent fixes
  (Ami-style copy block formatting).

- [#3](https://github.com/joachimhodana/dbt-doctor/pull/3) ([`f664ca2`](https://github.com/joachimhodana/dbt-doctor/commit/f664ca2)) - Fix the multiselect `a`-key select-all behavior in the interactive
  CLI prompt.

- [#4](https://github.com/joachimhodana/dbt-doctor/pull/4) ([`940e82a`](https://github.com/joachimhodana/dbt-doctor/commit/940e82a)) - Indent multi-line diagnostic help output in the CLI scan summary.

- [#5](https://github.com/joachimhodana/dbt-doctor/pull/5) ([`00f21ec`](https://github.com/joachimhodana/dbt-doctor/commit/00f21ec)) - Color diagnostic counts by severity in the CLI summary.

- [#6](https://github.com/joachimhodana/dbt-doctor/pull/6) ([`c55edc9`](https://github.com/joachimhodana/dbt-doctor/commit/c55edc9)) - Add severity icons to the summary counts.

- [#7](https://github.com/joachimhodana/dbt-doctor/pull/7) ([`8894a54`](https://github.com/joachimhodana/dbt-doctor/commit/8894a54)) - Frame the summary footer output (boxed border around the final
  score line).

- [#9](https://github.com/joachimhodana/dbt-doctor/pull/9) ([`a66c977`](https://github.com/joachimhodana/dbt-doctor/commit/a66c977)) - Move the website video overlays to a bottom-gradient layout and
  darken them.

- [`b5ea69b`](https://github.com/joachimhodana/dbt-doctor/commit/b5ea69b) - Add the GitHub Action for CI integration and fix monorepo scanning
  inside CI environments.

- [`578e75a`](https://github.com/joachimhodana/dbt-doctor/commit/578e75a) - Auto-install globally in the background when run via `npx`. (Later
  removed in 0.0.25 / #44 because it surprised users.)

- [`bde1167`](https://github.com/joachimhodana/dbt-doctor/commit/bde1167) - Add the video package and Ami skills for marketing-asset
  generation.
...
- [`19fa34b`](https://github.com/joachimhodana/dbt-doctor/commit/19fa34b) - Resolve a merge conflict in `cli.ts` introduced in 0.0.7.

- [`eef87e0`](https://github.com/joachimhodana/dbt-doctor/commit/eef87e0) - Use a single deeplink for `--fix` instead of the two-step deeplink

  - sleep dance.

- [`9cf691d`](https://github.com/joachimhodana/dbt-doctor/commit/9cf691d) / [`21c0d61`](https://github.com/joachimhodana/dbt-doctor/commit/21c0d61) - Skill install prompt copy refresh and README cleanup (remove the
...
- [`2ae9b87`](https://github.com/joachimhodana/dbt-doctor/commit/2ae9b87) - Fix the `--fix` deeplink to open the project with the correct cwd
...
- [`f9157c7`](https://github.com/joachimhodana/dbt-doctor/commit/f9157c7) - Add the `no-side-effect-in-get-handler` rule and export the oxlint
  plugin as a standalone entrypoint.

- [`9965871`](https://github.com/joachimhodana/dbt-doctor/commit/9965871) / [`80d676e`](https://github.com/joachimhodana/dbt-doctor/commit/80d676e) - Add the website with the animated CLI terminal landing page, and
  update the README with consumer-friendly docs.

- [`8379064`](https://github.com/joachimhodana/dbt-doctor/commit/8379064) / [`de1cbd4`](https://github.com/joachimhodana/dbt-doctor/commit/de1cbd4) - Add `fix` and `install-ami` commands that deeplink into Ami for
  automated rule application.

- [`330afc2`](https://github.com/joachimhodana/dbt-doctor/commit/330afc2) / [`7aa7b3f`](https://github.com/joachimhodana/dbt-doctor/commit/7aa7b3f) / [`05d2f79`](https://github.com/joachimhodana/dbt-doctor/commit/05d2f79) / [`4dfaeab`](https://github.com/joachimhodana/dbt-doctor/commit/4dfaeab) - Add ASCII doctor face / box branding to the CLI score output and
  the website terminal.

- [`b1f1abc`](https://github.com/joachimhodana/dbt-doctor/commit/b1f1abc) - Add the CI workflow for e2e tests, lint, and format.

- [`4b481c8`](https://github.com/joachimhodana/dbt-doctor/commit/4b481c8) - Add the React Doctor skill and the install prompt on first run.

## 0.0.5

### Patch Changes

- [`ccf404a`](https://github.com/joachimhodana/dbt-doctor/commit/ccf404a) - Gracefully handle failures in `oxlint`, the reduced-motion check,
  and summary-file writing so a single subsystem can't take down the
  scan.

- [`f1407d7`](https://github.com/joachimhodana/dbt-doctor/commit/f1407d7) - Gracefully handle knip failures on non-React config files.
...
- [`2d9a69b`](https://github.com/joachimhodana/dbt-doctor/commit/2d9a69b) - Add actionable help text, animation rules, per-rule summary files,
  and strengthen the framework / dependency detection.

- [`327c076`](https://github.com/joachimhodana/dbt-doctor/commit/327c076) - Move the website source into `packages/website`.

## 0.0.3

### Patch Changes

- [`680e7c4`](https://github.com/joachimhodana/dbt-doctor/commit/680e7c4) - Reduce default scan noisiness - tighter default rule severity
...
- [`a8770b7`](https://github.com/joachimhodana/dbt-doctor/commit/a8770b7) - Add CLI scaffolding with the initial oxlint integration: scan
...
- [`f50426b`](https://github.com/joachimhodana/dbt-doctor/commit/f50426b) - Initial publish - empty package scaffold to claim the npm name.
