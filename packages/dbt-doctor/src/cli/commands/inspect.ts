...
import type { Diagnostic, InspectResult } from "@dbt-doctor/types";
...
export const inspectAction = async (directory: string, flags: InspectFlags): Promise<void> => {
  ...
  try {
    ...
    if (redirectedDirectory && !isQuiet) {
      logger.dim(
        `Redirected to ${highlighter.info(toRelativePath(resolvedDirectory, requestedDirectory))} via dbt-doctor config "rootDir".`,
      );
      logger.break();
    }
