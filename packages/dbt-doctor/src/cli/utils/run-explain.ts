...
import type { Diagnostic, InspectOptions, DbtDoctorConfig } from "@dbt-doctor/types";
...
export const runExplain = async (
  ...
  if (matchingDiagnostics.length === 0) {
    logger.log(`No dbt-doctor diagnostics at ${filePath}:${line}.`);
    return;
  }
  ...
  for (const diagnostic of matchingDiagnostics) {
    ...
    if (diagnostic.suppressionHint) {
      ...
      logger.dim(
        "  No nearby dbt-doctor-disable-next-line comment was detected — add one immediately above this line to suppress.",
      );
    }
    logger.break();
  }
