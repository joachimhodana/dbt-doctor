...
describe("CLI inspect", () => {
  it("rejects --sarif with --json", () => {
    expect(() => validateModeFlags({ sarif: true, json: true })).toThrow(/--sarif cannot be combined/);
  });

  it("--sarif prints SARIF JSON", () => {
    const result = spawnSync(process.execPath, [cli, fixture, "--sarif", "--offline", "-y", "--fail-on", "none"], {
      encoding: "utf-8",
