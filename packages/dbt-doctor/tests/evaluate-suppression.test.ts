...
import { evaluateSuppression } from "@dbt-doctor/core";
...
describe("evaluateSuppression near-miss hints", () => {
  it("marks same-line disable-line comments as suppressed", () => {
    const lines = linesOf(
      `const x = 1; // dbt-doctor-disable-line dbt-doctor/no-derived-state-effect\n`,
    );
    const result = evaluateSuppression(lines, 0, "dbt-doctor/no-derived-state-effect");
  ...
  it("marks stacked disable-next-line comments as suppressed when each rule is listed separately", () => {
    const lines = linesOf(
      `// dbt-doctor-disable-next-line dbt-doctor/no-derived-state-effect\n` +
        `// dbt-doctor-disable-next-line dbt-doctor/no-fetch-in-effect\n` +
        `const x = 1;\n`,
    );
    expect(evaluateSuppression(lines, 2, "dbt-doctor/no-derived-state-effect")).toEqual({
      isSuppressed: true,
      nearMissHint: null,
    });
    expect(evaluateSuppression(lines, 2, "dbt-doctor/no-fetch-in-effect")).toEqual({
      isSuppressed: true,
      nearMissHint: null,
    });
  });
  ...
  it("returns null when no nearby disable-next-line comment exists", () => {
    ...
    expect(nearMissHintFor(lines, 1, "dbt-doctor/no-derived-state-effect")).toBeNull();
  });
  ...
  it("emits a wrong-rule hint when an adjacent comment lists different rules", () => {
    ...
    const hint = nearMissHintFor(lines, 1, "dbt-doctor/no-derived-state-effect");
    expect(hint).not.toBeNull();
    expect(hint).toContain("comma form");
    expect(hint).toContain("dbt-doctor/no-derived-state-effect");
  });
  ...
  it("emits a gap-code hint when a code line breaks the chain to the matching comment", () => {
    ...
    const hint = nearMissHintFor(lines, 2, "dbt-doctor/no-derived-state-effect");
    expect(hint).not.toBeNull();
    expect(hint).toContain("Move the comment");
    expect(hint).toContain("line 3");
  });

  it("supports SQL comment suppressions", () => {
    const lines = linesOf(
      `-- dbt-doctor-disable-next-line dbt-doctor/no-select-star\nselect id from users\n`,
    );
    expect(nearMissHintFor(lines, 1, "dbt-doctor/no-select-star")).toBeNull();
  });
  ...
  it("returns null when a description follows the matching rule list", () => {
    const lines = linesOf(
      `// dbt-doctor-disable-next-line dbt-doctor/no-derived-state-effect -- intentional fixture state mirror\nconst x = 1;\n`,
    );
    const result = evaluateSuppression(lines, 1, "dbt-doctor/no-derived-state-effect");
    expect(result).toEqual({ isSuppressed: true, nearMissHint: null });
  });
});
