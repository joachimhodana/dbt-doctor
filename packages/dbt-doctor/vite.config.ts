...
// which caused `dbt-doctor install` to print success while writing
...
const copySkillToDist = () => {
  const skillSource = path.resolve(packageRoot, "../../skills/dbt-doctor");
  const skillTarget = path.resolve(packageRoot, "dist/skills/dbt-doctor");
  ...
  pack: [
    {
      ...
      deps: { neverBundle: ["dbt-doctor-rules", "typescript"] },
      dts: true,
      target: "node22",
      platform: "node",
      env: {
      ...
      // bin/dbt-doctor.js, which owns the `#!/usr/bin/env node` line
      ...
      // `import "dbt-doctor"` consumers don't care either way — Node
