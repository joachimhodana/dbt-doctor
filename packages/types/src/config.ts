...
interface DbtDoctorIgnoreConfig {
  rules?: string[];
  files?: string[];
  overrides?: DbtDoctorIgnoreOverride[];
  tags?: string[];
}
 ...
 *
 * - `cli` — local terminal output from `dbt-doctor` (`printDiagnostics`).
 ...
 * `rules` and `categories` fields on `DbtDoctorConfig`. Per-rule
...
export interface SurfaceControls {
   ...
   * `"dbt-doctor/design-no-redundant-size-axes"`) to force-include.
   */
  includeRules?: string[];
...
export interface DbtDoctorConfig {
  ...
  offline?: boolean;
  /**
   * Redirect dbt-doctor at a different project directory than the one
   ...
   * (a warning is emitted and dbt-doctor falls back to the originally
   * requested directory).
   */
  rootDir?: string;
   ...
   *
   * Use this to teach dbt-doctor about custom auth guards in
   ...
   * and `// dbt-doctor-disable*` comments in source files. Default: `true`.
   *
   ...
   * neutralized so dbt-doctor reports every diagnostic regardless
   ...
   * toward the dbt-doctor score. Default: `true`.
   *
   ...
   * are NOT honored — dbt-doctor explicitly disables every oxlint
   ...
   *
   * Set to `false` to scan only dbt-doctor's curated rule set.
   */
   ...
   * (`"<plugin>/<rule>"`, e.g. `"dbt-doctor/no-array-index-as-key"`),
