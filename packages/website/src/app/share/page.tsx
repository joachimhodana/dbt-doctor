...
export const generateMetadata = async ({
  ...
  const title = `dbt Doctor - ${titlePrefix}Score: ${score}/100 (${label})`;
  ...
  const description =
    descriptionParts.length > 0
      ? `${descriptionParts.join(", ")} found. Run dbt-doctor on your dbt project to find SQL and modeling issues.`
      : "Run dbt-doctor on your dbt project to find SQL and modeling issues.";
...
const SharePage = async ({ searchParams }: { searchParams: Promise<ShareSearchParams> }) => {
  ...
  const projectLabel = projectName ? `${projectName} ` : "My dbt project ";
  const tweetText = `${projectLabel}scored ${score}/100 (${label}) on dbt Doctor. Run it on yours:`;
